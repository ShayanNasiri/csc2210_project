...
#SBATCH --mem=16G
#SBATCH -t 4:00:00
#SBATCH --output=results/system_e_val_%j.log

source scripts/setup_env.sh

# Evaluate all System E beta sweep weight files on validation set
for beta in 0.1 0.5 1.0 2.0; do
    weights="results/system_e_beta${beta}_weights.pt"
    if [ -f "$weights" ]; then
        echo "=========================================="
        echo "Evaluating System E with beta=$beta (val)"
        echo "=========================================="
...
echo "All System E val evaluations complete."
